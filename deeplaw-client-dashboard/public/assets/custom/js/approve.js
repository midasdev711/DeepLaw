        ...
        .then((res) => {
            if (res.data.status == "success") {
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    htmlcontent += 
                        `<div>
                            <a class='doclink' href=${".." + data[i]._doc.url.slice(10).replaceAll(" ", "%20")} target="_blank">${data[i]._doc.url.slice(16)}</a> 
                            <button class="btn btn-default approve" data-user='${data[i].user}' data-approver='${data[i].approver}' id="${data[i]._doc._id}">Approve</button>
                        </div><br/><br/>`;
                }
